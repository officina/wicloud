...
@Component({
    ...
    templateUrl: './order-wilamp.component.html'
})
export class OrderWilampComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.orderService.search({
                ...
                (res: HttpResponse<OrderWilamp[]>) => this.onSuccess(res.body, res.headers),
    ...
    trackId(index: number, item: OrderWilamp) {
        return item.id;
    }
    registerChangeInOrders() {
        this.eventSubscriber = this.eventManager.subscribe('orderListModification', (response) => this.reset());
    }
