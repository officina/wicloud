...
@Component({
    ...
    templateUrl: './installation-wilamp-list.component.html',
    styleUrls: ['installation-wilamp-detail.component.css']
})
export class InstallationWilampListComponent implements OnInit, OnDestroy {
    ...
    trackId(index: number, item: InstallationWilamp) {
        return item.id;
    }
    registerChangeInInstallations() {
        this.eventSubscriber = this.eventManager.subscribe('installationListModification', (response) => this.reset());
    }
    ...
    private onSuccess(data, headers) {
        ...
        for (let i = 0; i < data.length; i++) {
            this.installations.push(data[i]);
        }
    }
