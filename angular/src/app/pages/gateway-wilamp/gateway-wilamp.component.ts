...
@Component({
    selector: 'jhi-gateway-wilamp',
    templateUrl: './gateway-wilamp.component.html'
})
export class GatewayWilampComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/gateway-wilamp'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 1;
        this.currentSearch = '';
        this.router.navigate(['/gateway-wilamp', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: GatewayWilamp) {
        return item.id;
    }
