...
@Component({
    ...
    templateUrl: './shipping-wilamp.component.html',
})
export class ShippingWilampComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.shippingService.search({
                query: this.currentSearch,
                page: this.page,
                size: this.itemsPerPage,
                sort: this.sort(),
            }).subscribe(
                (res: HttpResponse<ShippingWilamp[]>) => this.onSuccess(res.body, res.headers),
    ...
    trackId(index: number, item: ShippingWilamp) {
        return item.id;
    }
    registerChangeInShippings() {
        this.eventSubscriber = this.eventManager.subscribe('shippingListModification', (response) => this.reset());
    }
