...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.gatewayInstallationRequestService.search({
                page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/gateway-installation-request'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: GatewayInstallationRequest) {
        return item.id;
    }
    registerChangeInGatewayInstallationRequests() {
        this.eventSubscriber = this.eventManager.subscribe('gatewayInstallationRequestListModification', (response) => this.loadAll());
    }
    ...
    private onSuccess(data, headers) {
        ...
        this.gatewayInstallationRequests = data;
    }
