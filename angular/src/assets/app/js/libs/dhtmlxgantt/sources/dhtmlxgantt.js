...
gantt = {version: "4.0.0"}, gantt.event = function (t, e, i) {
    ...
    getSum: function (t, e, i) {
        ...
        if (+i[0] < +gantt._min_date && n != a) {
            var r = Math.floor(e.width[0] * ((i[1] - gantt._min_date) / (i[1] - i[0])));
            s += e.width[0] - r, e.width[0] = r,
                i[0] = new Date(gantt._min_date)
        }
        var o = i.length - 1, _ = i[o], d = gantt.date.add(_, e.step, e.unit);
        if (+d > +gantt._max_date && o > 0) {
            var r = e.width[o] - Math.floor(e.width[o] * ((d - gantt._max_date) / (d - _)));
    ...
    drag: null, _events: {before_start: {}, before_finish: {}, after_finish: {}}, _handlers: {}, init: function () {
        ...
        var a = this._drag_task_coords(e, n), s = gantt.dateFromPos(a.start + i), r = gantt.dateFromPos(a.end + i);
        s ? r ? (e.start_date = s, e.end_date = r) : (e.end_date = new Date(gantt.getState().max_date), e.start_date = gantt.dateFromPos(gantt.posFromDate(e.end_date) - (a.end - a.start))) : (e.start_date = new Date(gantt.getState().min_date), e.end_date = gantt.dateFromPos(gantt.posFromDate(e.start_date) + (a.end - a.start)))
...
gantt._is_flex_task = function (t) {
    ...
    if (o == gantt._tasks.unit && r == gantt._tasks.step && +s >= +gantt._min_date && +s <= +gantt._max_date)a = Math.floor(gantt._day_index_by_date(s)), gantt._tasks.trace_x[a] || (a -= 1), n = new Date(gantt._tasks.trace_x[a]), i = new Date(n), i = gantt._tasks.trace_x[a + 1] ? new Date(gantt._tasks.trace_x[a + 1]) : gantt.date.add(n, r, o); else {
        for (a = Math.floor(gantt._day_index_by_date(s)), i = gantt.date[o + "_start"](new Date(this._min_date)), gantt._tasks.trace_x[a] && (i = gantt.date[o + "_start"](gantt._tasks.trace_x[a])); +s > +i;) {
