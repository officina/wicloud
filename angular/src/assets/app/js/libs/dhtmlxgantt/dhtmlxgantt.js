/*
 ...
 (c) Dinamenta, UAB.
 */
gantt = {version: "4.0.0"}, gantt.event = function (t, e, n) {
    ...
    t._silent_mode = !1, t._silentStart = function () {
        ...
        var e = [], n = function () {
            for (var n = !0, a = 0; a < e.length; a++)if (e[a]) {
    ...
    if (t.getBoundingClientRect) {
        var a = t.getBoundingClientRect(), i = document.body, s = document.documentElement, r = window.pageYOffset || s.scrollTop || i.scrollTop, o = window.pageXOffset || s.scrollLeft || i.scrollLeft, _ = s.clientTop || i.clientTop || 0, d = s.clientLeft || i.clientLeft || 0;
        return e = a.top + r - _,
            n = a.left + o - d, {y: Math.round(e), x: Math.round(n), width: t.offsetWidth, height: t.offsetHeight}
    }
    ...
    t = t || window.event;
    var e = t.target || t.srcElement, n = gantt.locate(t), a = !0;
    ...
    t = t || window.event;
    var e = t.target || t.srcElement, n = gantt.locate(e), a = gantt.locate(e, gantt.config.link_attribute), i = !gantt.checkEvent("onContextMenu") || gantt.callEvent("onContextMenu", [n, a, t]);
    ...
    t = t || window.event;
    var e = t.target || t.srcElement, n = gantt.locate(t), a = !gantt.checkEvent("onTaskDblClick") || gantt.callEvent("onTaskDblClick", [n, t]);
    if (a) {
    ...
    e && (this._settings = e), gantt._eventable(this), gantt.event(t, "mousedown", gantt.bind(function (n) {
        e.original_target = {target: n.target || n.srcElement}, this.dragStart(t, n)
    }, this))
    ...
    dragStart: function (t, e) {
        ...
        if (!this.config.marker && !this.config.started) {
            var n = this.getPosition(e), a = n.x - this.config.pos.x, i = n.y - this.config.pos.y, s = Math.sqrt(Math.pow(Math.abs(a), 2) + Math.pow(Math.abs(i), 2));
    ...
    this._click.gantt_close = this.bind(function (t, e, n) {
        ...
        if (this.callEvent("onGridHeaderClick", [a, t])) {
            ...
            if (this.config.sort) {
                for (var i, s = a, r = 0; this.config.columns.length; r++)if (this.config.columns[r].name == a) {
                    i = this.config.columns[r];
                    break
                }
                if (i && void 0 !== i.sort && i.sort !== !0 && (s = i.sort,
                        !s))return;
                var o = this._sort && this._sort.direction && this._sort.name == a ? this._sort.direction : "desc";
                o = "desc" == o ? "asc" : "desc", this._sort = {name: a, direction: o}, this.sort(s, "desc" == o)
            }
        }
    ...
    if (this.config.autofit || n.length) {
        ...
        if (n.length > 0)for (var o = r / (n.length ? n.length : 1), i = 0; i < n.length; i++) {
            var _ = n[i];
            a[_] += o
        } else for (var o = r / (a.length ? a.length : 1), i = 0; i < a.length; i++)a[i] += o;
    ...
    for (var t = this.getGridColumns(), e = [], n = 0, a = this.locale.labels, i = this.config.scale_height - 2, s = 0; s < t.length; s++) {
        ...
        var d = this._sort && o.name == this._sort.name ? "<div class='gantt_sort gantt_" + this._sort.direction + "'></div>" : "", l = ["gantt_grid_head_cell", "gantt_grid_head_" + o.name, r ? "gantt_last_cell" : "", this.templates.grid_header_class(o.name, o)].join(" "), g = "width:" + (_ - (r ? 1 : 0)) + "px;", h = o.label || a["column_" + o.name];
        h = h || "";
    ...
    for (var e = this.getGridColumns(), n = [], a = 0; a < e.length; a++) {
        ...
        var _ = "gantt_cell" + (r ? " gantt_last_cell" : ""), d = "";
        if (o.tree) {
    ...
    if (t != n) {
        ...
        var i = this.getTask(t), s = this.getParent(i.id), r = (this.getChildren(this.getParent(i.id)), this.getChildren(n));
    ...
    this.defined(this.config.dnd_sensitivity) && (t.config.sensitivity = this.config.dnd_sensitivity),
        t.attachEvent("onBeforeDragStart", this.bind(function (e, n) {
            ...
            var i = this.locate(n), s = gantt.getTask(i);
        ...
        var e = this._get_position(this.$grid_data), n = e.x, a = t.pos.y - 10;
        ...
        var i = t.config, s = t._getGridPos(n);
        i.marker.style.left = s.x + 10 + "px", i.marker.style.top = s.y + "px", s = t._getGridPos(n);
        var r = (s.x, s.y), o = document.documentElement.scrollTop || document.body.scrollTop, _ = document.documentElement.scrollLeft || document.body.scrollLeft, d = document.elementFromPoint(s.x - _ + 1, r - o), l = this.locate(d), g = this.getTask(t.config.id);
        if (this.isTaskExists(l) || (l = t.lastTaskOfLevel(gantt.config.order_branch_free ? g.$level : 0),
            ...
            var h = gantt._get_position(d), c = this.getTask(l);
            if (h.y + d.offsetHeight / 2 < r) {
                var u = this.getGlobalTaskIndex(c.id), f = this._pull[this._order[u + 1]];
                if (f) {
                    if (f.id == g.id)return;
                    c = f
                } else if (f = this._pull[this._order[u]], a(f, g) && f.id != g.id)return void this.moveTask(g.id, -1, this.getParent(f.id))
            }
            for (var u = this.getGlobalTaskIndex(c.id), p = this._pull[this._order[u - 1]], v = 1; (!p || p.id == c.id) && u - v >= 0;)p = this._pull[this._order[u - v]], v++;
            if (g.id == c.id)return;
    ...
    getSum: function (t, e, n) {
        ...
        return gantt._init_template("date_scale"), {
            unit: gantt.config.scale_unit, step: gantt.config.step, template: gantt.templates.date_scale,
            date: gantt.config.date_scale, css: gantt.templates.scale_cell_class
        }
        ...
        var e = gantt.mixin({count: 0, col_width: 0, full_width: 0, height: 0, width: [], left: [], trace_x: []}, t);
        return this.eachColumn(t.unit, t.step, function (t) {
            e.count++, e.trace_x.push(new Date(t))
        }), e
    }, iterateScales: function (t, e, n, a, i) {
        for (var s = e.trace_x, r = t.trace_x, o = n || 0, _ = a || r.length - 1, d = 0, l = 1; l < s.length; l++)for (var g = o; _ >= g; g++)+r[g] != +s[l] || (i && i.apply(this, [d, l, o, g]), o = g, d = l)
    }, alineScaleColumns: function (t, e, n, a) {
        this.iterateScales(t, e, n, a, function (n, a, i, s) {
            var r = this.getSum(t.width, i, s - 1), o = this.getSum(e.width, n, a - 1);
            o != r && this.setSumWidth(r, e, n, a - 1)
        })
    }, eachColumn: function (t, e, n) {
        var a = new Date(gantt._min_date), i = new Date(gantt._max_date);
        ...
        var e = t.trace_x, n = 0, a = t.width.length - 1, i = 0;
        ...
        var r = e.length - 1, o = e[r], _ = gantt.date.add(o, t.step, t.unit);
    ...
    drag: null, _events: {before_start: {}, before_finish: {}, after_finish: {}}, _handlers: {}, init: function () {
        ...
        var a = gantt.config, i = this._drag_task_coords(t, n);
        ...
        var a = this._drag_task_coords(t, n), i = Math.max(0, n.pos.x - a.start);
        t.progress = Math.min(1, i / (a.end - a.start))
    }, _move: function (t, e, n) {
        var a = this._drag_task_coords(t, n), i = gantt.dateFromPos(a.start + e), s = gantt.dateFromPos(a.end + e);
        ...
        var n = e.obj_s_x = e.obj_s_x || gantt.posFromDate(t.start_date), a = e.obj_e_x = e.obj_e_x || gantt.posFromDate(t.end_date);
        ...
        if (e.mode) {
            ...
            var i = n.x - e.start_x, s = gantt.getTask(e.id);
            if (this._handlers[e.mode]) {
                var r = gantt.mixin({}, s), o = gantt.mixin({}, s);
        ...
        if (2 != t.button) {
            ...
            if (gantt.isTaskExists(n) && (a = gantt.getTask(n)), !gantt._is_readonly(a) && !this.drag.mode) {
                ...
                } else this.clear_drag_state(); else if (gantt.checkEvent("onMouseDown") && gantt.callEvent("onMouseDown", [i.split(" ")[0]]) && e.parentNode)return this.on_mouse_down(t, e.parentNode);
            }
        }
        ...
        var i = gantt._tasks.unit, s = gantt._tasks.step;
        ...
        var e = gantt.config.drag_mode, n = (t || "").split(" "), a = n[0], i = {mode: null, left: null};
        switch (a) {
    ...
    e || (e = {mode: gantt.config.drag_mode.move}), gantt._tasks_dnd._fix_dnd_scale_time(t, e)
    ...
    var i = null;
    if (i = "object" == typeof t ? t : {source: t, target: e, type: this._get_link_type(n, a)}, !i)return !1;
    ...
    var e = this._path_builder.get_points(t), n = gantt._drawer, a = n.get_lines(e), i = document.createElement("div"), s = "gantt_task_link";
    ...
    var d = a[a.length - 1].direction, l = gantt._render_link_arrow(e[e.length - 1], d);
    return t.color && (l.style.borderColor = t.color), i.appendChild(l), i
}, gantt._render_link_arrow = function (t, e) {
    var n = document.createElement("div"), a = gantt._drawer, i = t.y, s = t.x, r = gantt.config.link_arrow_size, o = gantt.config.row_height, _ = "gantt_link_arrow gantt_link_arrow_" + e;
    switch (e) {
        case a.dirs.right:
            i -= (r - o) / 2, s -= r;
            break;
    ...
    current_pos: null, dirs: {left: "left", right: "right", up: "up", down: "down"}, path: [], clear: function () {
        ...
        var e = gantt.copy(t), n = this.current_pos, a = this._get_line(n, e);
        this.path.push(a), this.current_pos = e
    }, get_path: function () {
        return this.path
    }, get_wrapper_sizes: function (t) {
        var e, n = gantt.config.link_wrapper_width, a = (gantt.config.link_line_width,
        t.y + (gantt.config.row_height - n) / 2);
        switch (t.direction) {
            case this.dirs.left:
                e = {top: a, height: n, lineHeight: n, left: t.x - t.size - n / 2, width: t.size + n};
                break;
            case this.dirs.right:
                e = {top: a, lineHeight: n, height: n, left: t.x - n / 2, width: t.size + n};
                break;
            case this.dirs.up:
                e = {top: a - t.size, lineHeight: t.size + n, height: t.size + n, left: t.x - n / 2, width: n};
                break;
            case this.dirs.down:
                e = {top: a, lineHeight: t.size + n, height: t.size + n, left: t.x - n / 2, width: n}
        }
        return e
    }, get_line_sizes: function (t) {
        var e, n = gantt.config.link_line_width, a = gantt.config.link_wrapper_width, i = t.size + n;
        switch (t.direction) {
            case this.dirs.left:
            case this.dirs.right:
                e = {height: n, width: i, marginTop: (a - n) / 2, marginLeft: (a - n) / 2};
                break;
            case this.dirs.up:
            case this.dirs.down:
                e = {height: i, width: n, marginTop: (a - n) / 2, marginLeft: (a - n) / 2}
        }
        return e
    }, render_line: function (t) {
        var e = this.get_wrapper_sizes(t), n = document.createElement("div");
        n.style.cssText = ["top:" + e.top + "px", "left:" + e.left + "px", "height:" + e.height + "px", "width:" + e.width + "px"].join(";"), n.className = "gantt_line_wrapper";
        var a = this.get_line_sizes(t), i = document.createElement("div");
        ...
        var n = this.get_direction(t, e), a = {x: t.x, y: t.y, direction: this.get_direction(t, e)};
    ...
    path: [], clear: function () {
        ...
        var e = this.get_endpoint(t), n = gantt.config, a = e.e_y - e.y, i = e.e_x - e.x, s = gantt._drawer.dirs;
        this.clear(), this.point({x: e.x, y: e.y});
        var r = 2 * n.link_arrow_size, o = e.e_x > e.x;
        if (t.type == gantt.config.links.start_to_start)this.point_to(s.left, r), o ? (this.point_to(s.down, a), this.point_to(s.right, i)) : (this.point_to(s.right, i), this.point_to(s.down, a)), this.point_to(s.right, r); else if (t.type == gantt.config.links.finish_to_start)if (o = e.e_x > e.x + 2 * r, this.point_to(s.right, r), o)i -= r, this.point_to(s.down, a), this.point_to(s.right, i); else {
            i -= 2 * r;
            var _ = a > 0 ? 1 : -1;
        ...
        } else if (t.type == gantt.config.links.finish_to_finish)this.point_to(s.right, r), o ? (this.point_to(s.right, i), this.point_to(s.down, a)) : (this.point_to(s.down, a), this.point_to(s.right, i)), this.point_to(s.left, r); else if (t.type == gantt.config.links.start_to_finish)if (o = e.e_x > e.x - 2 * r, this.point_to(s.left, r), o) {
            i += 2 * r;
            var _ = a > 0 ? 1 : -1;
        ...
        var e = gantt.config.links, n = !1, a = !1;
        ...
        var i = gantt._get_task_visible_pos(gantt._pull[t.source], n), s = gantt._get_task_visible_pos(gantt._pull[t.target], a);
    ...
    function s(t, e, n, i) {
        var s = _(), d = a(), l = ["gantt_link_direction"];
        ...
        if (g = Math.max(0, g - 3)) {
            s.className = l.join(" ");
            var h = (i - e) / (n - t), c = Math.atan(h);
            2 == o(t, n, e, i) ? c += Math.PI : 3 == o(t, n, e, i) && (c -= Math.PI);
            var u = Math.sin(c), f = Math.cos(c), p = Math.round(e), v = Math.round(t), m = ["-webkit-transform: rotate(" + c + "rad)", "-moz-transform: rotate(" + c + "rad)", "-ms-transform: rotate(" + c + "rad)", "-o-transform: rotate(" + c + "rad)", "transform: rotate(" + c + "rad)", "width:" + Math.round(g) + "px"];
            if (-1 != window.navigator.userAgent.indexOf("MSIE 8.0")) {
                m.push('-ms-filter: "' + r(u, f) + '"');
                var k = Math.abs(Math.round(t - n)), b = Math.abs(Math.round(i - e));
                switch (o(t, n, e, i)) {
                    case 1:
                        p -= b;
                        break;
    ...
    }), g = "task_left", h = "task_right", c = "gantt_link_point", u = "gantt_link_control";
    l.attachEvent("onBeforeDragStart", gantt.bind(function (e, n) {
        ...
        if (gantt._locate_css(a, c)) {
            ...
            var s = gantt._link_source_task = this.locate(n), r = gantt.getTask(s);
        ...
        var r = l.config, o = l.getPosition(i);
        n(r.marker, o);
        var _ = gantt._is_link_drop_area(i), d = gantt._link_target_task, g = gantt._link_landing, c = gantt._link_target_task_start, f = gantt.locate(i), p = !0;
        if (_ && (p = !gantt._locate_css(i, h), _ = !!f), gantt._link_target_task = f, gantt._link_landing = _, gantt._link_target_task_start = p, _) {
            var v = gantt.getTask(f), m = gantt._locate_css(i, u), k = 0;
        ...
        if (t.from && t.to && t.from != t.to) {
            var e = gantt._get_link_type(t.from_start, t.to_start), n = {source: t.from, target: t.to, type: e};
    ...
    n && n.setAttribute(this.config.layer_attribute, !0), this._task_area_renderers[t] = {
        ...
        }, rendered: this._task_area_pulls[t], node: n, unload: function () {
    ...
    this._tasks = {
        ...
        var n = gantt._get_mouse_pos(t), a = gantt.dateFromPos(n.x), i = Math.floor(gantt._day_index_by_date(a)), s = gantt._tasks.trace_x[i];
        ...
        var e = this.locate(t), a = this.getTask(e), i = null;
    ...
    if (this.config.start_date && this.config.end_date) {
        ...
        var e = new Date(this.config.end_date), n = this.date[t + "_start"](new Date(e));
    ...
    (t.template || t.date) && (a = t.template || this.date.date_to_str(t.date)), i = t.css || function () {
        }, !t.css && this.config.inherit_scale_class && (i = gantt.templates.scale_cell_class);
    for (var s = 0; s < t.count; s++) {
        n = new Date(t.trace_x[s]);
        var r = a.call(this, n), o = t.width[s], _ = "", d = "", l = "";
        if (o) {
    ...
    if (this._is_chart_visible()) {
        var i = this._scale_helpers, s = this._get_scales();
        ...
        for (var r = this._get_resize_options(), o = r.x ? Math.max(this.config.autosize_min_width, 0) : this.$task.offsetWidth, _ = i.prepareConfigs(s, this.config.min_column_width, o, a), d = this._tasks = _[_.length - 1], l = [], g = this.templates.scale_row_class, h = 0; h < _.length; h++) {
            var c = "gantt_scale_line", u = g(_[h]);
    ...
    var e = gantt._tasks, n = e.count, a = document.createElement("div");
    ...
    var _ = t.$index % 2 !== 0, d = gantt.templates.task_row_class(t.start_date, t.end_date, t), l = "gantt_task_row" + (_ ? " odd" : "") + (d ? " " + d : "");
    ...
    if (this._is_render_active()) {
        var n = this._get_task_renderers(), a = this.getTask(t);
    ...
    if (this._is_render_active())if (this.isLinkExists(t))this._render_link(t); else for (var e = this._get_link_renderers(), n = 0; n < e.length; n++)e[n].remove_item(t)
    ...
    if (r.link_source_id == n && a.push("gantt_link_source"), r.link_target_id == n && a.push("gantt_link_target"), this.config.highlight_critical_path && this.isCriticalTask && this.isCriticalTask(s) && a.push("gantt_critical_task"), r.link_landing_area && r.link_target_id && r.link_source_id && r.link_target_id != r.link_source_id) {
        var o = r.link_source_id, _ = r.link_source_start, d = r.link_target_start, l = gantt.isLinkAllowed(o, n, _, d), g = "";
    ...
    +n.end_date <= +i.max_date && t.appendChild(a(e + " task_right")), +n.start_date >= +i.min_date && t.appendChild(a(e + " task_left"))
    ...
    if (!this._isAllowedUnscheduledTask(t)) {
        var e = this._get_task_pos(t), n = this.config, a = this._get_task_height(), i = Math.floor((this.config.row_height - a) / 2);
        this._get_safe_type(t.type) == n.types.milestone && n.link_line_width > 1 && (i += 1);
        var s = document.createElement("div"), r = gantt._get_task_width(t), o = this._get_safe_type(t.type);
    ...
    var e = this.config.type_renderers, n = e[this._get_safe_type(t.type)], a = this._task_default_render;
    ...
    for (var a in n)for (var i = e[a], s = 0; s < i.length; s++)for (var r = gantt.getLink(i[s]), o = 0; o < n[a].length; o++)if (r.type == n[a][o])return "gantt_link_crossing";
    return ""
    ...
    n = Math.max(n - 2, 0);
    var i = document.createElement("div"), s = Math.round(n * a);
    ...
    var n = this._findBinary(this._tasks.left, t), a = this._tasks.left[n], i = e.width[n] || e.col_width, s = 0;
    i && (s = (t - a) / i);
    var r = 0;
    ...
    var n = Math.floor(e), a = e % 1, i = gantt._tasks.left[Math.min(n, gantt._tasks.width.length - 1)];
    ...
    var e = new Date(t).valueOf(), n = gantt._tasks.trace_x, a = gantt._tasks.ignore_x;
    ...
    for (var n, a, i, s = 0, r = t.length - 1; r >= s;)if (n = Math.floor((s + r) / 2), a = +t[n], i = +t[n - 1], e > a)s = n + 1; else {
        if (!(a > e))return n;
    ...
    var n = gantt.env.isIE ? document.documentElement : document.body, e = {
    ...
    prepareConfig: function (t) {
        ...
        return {
            ...
            container: t, getRenderers: function () {
                var t = [];
                ...
                if (t && this.tempCollection.push(t), this.container())for (var n = this.container(), a = this.tempCollection, i = 0; i < a.length; i++) {
                    var t = a[i], s = t.container, r = t.id, o = t.topmost;
                    if (!s.parentNode)if (o)n.appendChild(s); else {
...
gantt.removeLinkLayer = function (t) {
    ...
    var n = t.xmlDoc.responseText, a = this[e].parse(n, t);
    ...
    for (var n in t)if (t.hasOwnProperty(n)) {
        e = !0;
        var a = t[n], i = this.serverList[n];
        if (!i)continue;
        i.splice(0, i.length);
        for (var s = 0; s < a.length; s++) {
            var r = a[s], o = this.copy(r);
            o.key = o.value;
    ...
    if (n)for (var a = 0; a < n.length; a++)this._sync_order_item(this._pull[n[a]], e || !t.$open)
    ...
    parse: function (t) {
        ...
        }, gantt.config.root_id, this), e = gantt._links.slice(), {data: t, links: e}
    }
},gantt.xml = {
    _xmlNodeToJSON: function (t, e) {
        ...
        for (var e = {}, n = gantt.ajax.xpath("//coll_options", t), a = 0; a < n.length; a++)for (var i = n[a].getAttribute("for"), s = e[i] = [], r = gantt.ajax.xpath(".//item", n[a]), o = 0; o < r.length; o++) {
    ...
    parse: function (t, e) {
        ...
        for (var n = {collections: {links: []}}, a = n.data = [], i = gantt.ajax.xpath("//task", e), s = 0; s < i.length; s++) {
        ...
        for (var s = 0; s < a.length; s++) {
            ...
            o.start_date = o.startdate || o.est, o.end_date = o.enddate, o.text = o.name, o.duration = o.duration / 8, o.open = 1, o.duration || o.end_date || (o.duration = 1), o.predecessortasks && n.collections.links.push({
                target: o.id, source: o.predecessortasks,
    ...
    _working_units_cache: {
        _cache: {}, get: function (t, e) {
            var n = -1, a = this._cache;
            if (a && a[t]) {
                var i = a[t], s = e.getTime();
                void 0 !== i[s] && (n = i[s]);
            }
            return n
        }, put: function (t, e, n) {
            if (!t || !e)return !1;
            var a = this._cache, i = e.getTime();
    ...
    set_time: function (t) {
        var e = void 0 !== t.hours ? t.hours : !0, n = this._timestamp(t);
    ...
    get_working_hours: function (t) {
        var e = this._timestamp({date: t}), n = !0;
    ...
    next_date: function (t, e, n) {
        var a = +t, i = e + "_" + n, s = this.intern_dates_pull[i];
    ...
    add_worktime: function (t, e, n, a) {
        if (!n)return !1;
        var i = new Date(t), s = 0, a = a || 1, e = 1 * e;
    ...
    get_closest_worktime: function (t) {
        ...
        var e = t.unit, n = gantt.date[e + "_start"](t.date), a = new Date(n), i = new Date(n), s = !0, r = 3e3, o = 0, _ = "any" == t.dir || !t.dir, d = 1;
    ...
    for (var a = this.getChildren(n), i = !1, s = 0, r = a.length; r > s; s++)if (a[s] == t.id) {
        i = !0;
        break
    }
...
gantt._sync_parent = function (t) {
    ...
    var n = e && e != this.config.root_id ? this.getTask(e) : !1, a = "";
    ...
    var n = this._dp && !e, a = "", i = n ? "off" != this._dp.updateMode : !1;
    ...
    var a = this._working_time_helper, i = e >= 0 ? 1 : -1;
...
gantt._is_flex_task = function (t) {
    ...
    var e = null, n = null, a = void 0 !== t ? t : gantt.config.root_id;
    ...
    }, a), {start_date: e ? new Date(e) : null, end_date: n ? new Date(n) : null}
    ...
    if (t) {
        var n = this.getTask(t), a = this.getParent(n), i = !0;
        if (n.$no_start || n.$no_end) {
            var s = n.start_date.valueOf(), r = n.end_date.valueOf();
    ...
    t instanceof Date && (t = {date: t, unit: gantt._tasks.unit, step: gantt._tasks.step});
    var e, n, a, i = t.date, s = t.step, r = t.unit;
    if (r == gantt._tasks.unit && s == gantt._tasks.step && +i >= +gantt._min_date && +i <= +gantt._max_date)a = Math.floor(gantt._day_index_by_date(i)), gantt._tasks.trace_x[a] || (a -= 1), n = new Date(gantt._tasks.trace_x[a]), e = new Date(n), e = gantt._tasks.trace_x[a + 1] ? new Date(gantt._tasks.trace_x[a + 1]) : gantt.date.add(n, s, r); else {
    ...
    for (var e = this.getSiblings(t), n = 0, a = e.length; a > n; n++)if (e[n] == t)return e[n + 1] || null;
    return null
    ...
    t.setTransactionMode("POST", !0), t.serverProcessor += (-1 != t.serverProcessor.indexOf("?") ? "&" : "?") + "editing=true", t._serverProcessor = t.serverProcessor, t.styles = {
        ...
        t._ganttMode && (n[t._ganttMode] = {
            _in_progress: t._in_progress, _invalid: t._invalid,
    ...
    this.attachEvent("onTaskIdChange", function (a, i) {
        if (t._waitMode) {
            ...
            var _ = this.getTask(i), d = this._get_task_links(_);
            if (d.length) {
                n = n || {};
    ...
    t.afterUpdate = function () {
        ...
        var n = t._ganttMode, i = e.filePath;
        ...
        var a;
        a = "tasks" == t._ganttMode ? this.isTaskExists(e) ? this.getTask(e) : {id: e} : this.isLinkExists(e) ? this.getLink(e) : {
            id: e
        }, a = gantt.copy(a);
        var i = {};
    ...
    if (!gantt.skin || t)for (var e = document.getElementsByTagName("link"), n = 0; n < e.length; n++) {
...
gantt._lightbox_template = "<div class='gantt_cal_ltitle'><span class='gantt_mark'>&nbsp;</span><span class='gantt_time'></span><span class='gantt_title'></span></div><div class='gantt_cal_larea'></div>",gantt.showLightbox = function (t) {
    ...
    for (var n = this._get_typed_lightbox_config(), a = 0; a < n.length; a++)if (n[a].map_to == t)for (var i = n[a].options, s = 0; s < i.length; s++)if (i[s].key == e)return i[s].label;
    return ""
    ...
    for (var e = "", n = 0; n < t.length; n++) {
        ...
        if (a) {
            ...
            var i = t[n].hidden ? " style='display:none'" : "", s = "";
    ...
    if (t) {
        ...
        var e = window.pageYOffset || document.body.scrollTop || document.documentElement.scrollTop, n = window.pageXOffset || document.body.scrollLeft || document.documentElement.scrollLeft, a = window.innerHeight || document.documentElement.clientHeight;
    ...
    gantt.lightbox_events = {}, gantt.lightbox_events.gantt_save_btn = function (t) {
        ...
        if (e.getAttribute("dhx_button"))gantt.callEvent("onLightboxButton", [e.className, e, t]); else {
            var n, a, i, s = gantt._getClassName(e);
            -1 != s.indexOf("gantt_custom_button") && (-1 != s.indexOf("gantt_custom_button_") ? (n = e.parentNode.getAttribute("index"), i = e.parentNode.parentNode) : (n = e.getAttribute("index"), i = e.parentNode, e = e.firstChild));
        ...
        t = t || window.event;
        var e = t.target ? t.target : t.srcElement, n = gantt._getClassName(e);
    ...
    var e = t.map_to, n = {time: !0, time_optional: !0, duration: !0, duration_optional: !0};
    ...
    for (var e = this._get_typed_lightbox_config(), n = 0; n < e.length; n++) {
        ...
        if (i) {
            var s = i.get_value.call(this, a, t, e[n]), r = gantt._resolve_default_mapping(e[n]);
            if ("string" == typeof r && "auto" != r)t[r] = s; else if ("object" == typeof r)for (var o in r)r[o] && (t[r[o]] = s[o])
        }
    }
    return t
    ...
    var n = t, a = e.getElementsByTagName("span");
    ...
    for (var i = this._get_typed_lightbox_config(this.getLightboxType()), s = 0; s < i.length; s++) {
        ...
        if (this.form_blocks[r.type]) {
            var o = document.getElementById(r.id).nextSibling, _ = this.form_blocks[r.type], d = gantt._resolve_default_mapping(i[s]), l = this.defined(n[d]) ? n[d] : r.default_value;
    ...
    var e = this._get_typed_lightbox_config(), n = 0;
    for (n; n < e.length && e[n].name != t; n++);
    var a = e[n];
    if (!a)return null;
    this._lightbox || this.getLightbox();
    var i = document.getElementById(a.id), s = i.nextSibling, r = {
        section: a,
        header: i,
        node: s,
        getValue: function (t) {
    ...
    if (gantt._dnd_start_lb) {
        ...
        var e = gantt.getLightbox(), n = t && t.target ? [t.pageX, t.pageY] : [event.clientX, event.clientY];
    ...
    if (t && t.focus)if (gantt.config.touch); else try {
    ...
    getTimePicker: function (t, e) {
        ...
        var a = this.config, i = this.date.date_part(new Date(gantt._min_date.valueOf())), s = 1440, r = 0;
        ...
        for (var o = "", _ = 0; _ < n.length; _++) {
            ...
            if (l) {
                var k = t.readonly ? "disabled='disabled'" : "", b = e ? " style='display:none'" : "";
        ...
        render: function (t) {
            var e = this.form_blocks.getTimePicker.call(this, t), n = ["<div style='height:" + (t.height || 30) + "px;padding-top:0px;font-size:inherit;text-align:center;' class='gantt_section_time'>"];
            ...
            var i = a, s = t.getElementsByTagName("select"), r = a._time_format_order;
            a._time_format_size;
            if (i.auto_end_date)for (var o = function () {
            ...
            var l = n[d.start_date] || new Date, g = n[d.end_date] || gantt.calculateEndDate(l, 1);
            this.form_blocks._fill_lightbox_select(s, 0, l, r, i), this.form_blocks._fill_lightbox_select(s, r.size, g, r, i)
        }, get_value: function (t, e, n) {
            var a = t.getElementsByTagName("select"), i = n._time_format_order, s = 0, r = 0;
            ...
            var l = gantt._resolve_default_mapping(n), g = {start_date: new Date(_), end_date: new Date(d)};
        ...
        render: function (t) {
            ...
            var n = this.locale.labels[this.config.duration_unit + "s"], a = t.single_date ? ' style="display:none"' : "", i = t.readonly ? " disabled='disabled'" : "", s = "<div class='gantt_duration' " + a + "><input type='button' class='gantt_duration_dec' value='-'" + i + "><input type='text' value='5' class='gantt_duration_value'" + i + "><input type='button' class='gantt_duration_inc' value='+'" + i + "> " + n + " <span></span></div>", r = "<div style='height:" + (t.height || 30) + "px;padding-top:0px;font-size:inherit;' class='gantt_section_time'>" + e + " " + s + "</div>";
            return r
        }, set_value: function (t, e, n, a) {
            function i() {
                var e = gantt.form_blocks.duration._get_start_date.call(gantt, t, a), n = gantt.form_blocks.duration._get_duration.call(gantt, t, a), i = gantt.calculateEndDate(e, n);
            ...
            var r = a, o = t.getElementsByTagName("select"), _ = t.getElementsByTagName("input"), d = _[1], l = [_[0], _[2]], g = t.getElementsByTagName("span")[0], h = a._time_format_order;
            ...
            var u = n[c.start_date] || new Date, f = n[c.end_date] || gantt.calculateEndDate(u, 1), p = Math.round(n[c.duration]) || gantt.calculateDuration(u, f);
            gantt.form_blocks._fill_lightbox_select(o, 0, u, h, r), d.value = p, i()
        }, _get_start_date: function (t, e) {
            var n = t.getElementsByTagName("select"), a = e._time_format_order, i = 0, s = 0;
            if (gantt.defined(a[3])) {
                var r = n[a[3]], o = parseInt(r.value, 10);
            ...
            var a = gantt.form_blocks.duration._get_start_date(t, n), i = gantt.form_blocks.duration._get_duration(t, n), s = gantt.calculateEndDate(a, i), r = gantt._resolve_default_mapping(n), o = {
    ...
    if (this.getState().lightbox) {
        var n = this.getState().lightbox, a = this.getLightboxValues(), i = this.copy(this.getTask(n));
        this.resetLightbox();
        var s = this.mixin(i, a, !0), r = this.getLightbox(t ? t : void 0);
    ...
    var e = t, n = {
        render: e.render, focus: e.focus, set_value: function (t, a, i, s) {
            ...
            return a.disabled ? {start_date: null} : e.get_value.call(gantt, t, n, a)
            ...
            if (gantt.callEvent("onSectionToggle", [gantt._lightbox_id, e]), t.style.display = e.disabled ? "none" : "block", e.button) {
                var n = t.previousSibling.firstChild.firstChild, a = gantt.locale.labels, i = e.disabled ? a[e.name + "_enable_button"] : a[e.name + "_disable_button"];
    ...
    setTransactionMode: function (t, e) {
        ...
        if (t) {
            ...
            var n = this, a = function (a) {
                var i = [];
                if (e)i.push(e); else if (t)for (var s in t)i.push(s);
            ...
            if ("GET" == this._tMode)gantt.ajax.get(i + (-1 != i.indexOf("?") ? "&" : "?") + this.serialize(t, e), a); else if ("POST" == this._tMode)gantt.ajax.post(i, this.serialize(t, e), a); else if ("REST" == this._tMode) {
                var s = this.getState(e), r = i.replace(/(\&|\?)editing\=true/, ""), o = "", _ = "post";
                if ("inserted" == s ? o = this.serialize(t, e) : "deleted" == s ? (_ = "DELETE", r = r + ("/" == r.slice(-1) ? "" : "/") + e) : (_ = "PUT", o = this.serialize(t, e), r = r + ("/" == r.slice(-1) ? "" : "/") + e), this._payload)for (var d in this._payload)r = r + this._urlSeparator(r) + this.escape(d) + "=" + this.escape(this._payload[d]);
                gantt.ajax.query({url: r, method: _, headers: this._headers, data: o, callback: a})
            }
            this._waitMode++
        }
        ...
        if (window.JSON)try {
            var a = JSON.parse(e.xmlDoc.responseText), i = a.action || this.getState(n) || "updated", s = a.sid || n[0], r = a.tid || n[0];
        ...
        for (var l = 0; l < d.length; l++) {
            var g = d[l], i = g.getAttribute("type"), s = g.getAttribute("sid"), r = g.getAttribute("tid");
        ...
        var t = this, e = this.obj.getUserData(0, "version"), n = this.serverProcessor + gantt._urlSeparator(this.serverProcessor) + ["dhx_user=" + this._user, "dhx_version=" + e].join("&");
        n = n.replace("editing=true&", ""), this.getUpdates(n, function (e) {
            ...
            if (a.length) {
                ...
                for (var i = 0; i < a.length; i++) {
                    var s = a[i].getAttribute("status"), r = a[i].getAttribute("id"), o = a[i].getAttribute("parent");
                    switch (s) {
                        case"inserted":
    ...
    var i = t.data || gantt.xml._xmlNodeToJSON(t.firstChild), s = {add: gantt.addTask, isExist: gantt.isTaskExists};
    ...
    buttons: {
        edit: function (t) {
            ...
            var e = gantt.locale.labels.confirm_deleting, n = gantt.locale.labels.confirm_deleting_title;
    ...
    var t = this.config.scale_height, e = this._order.length * this.config.row_height, n = this._scroll_hor ? this.config.scroll_size + 1 : 0;
    ...
    var e = ["width", "height", "paddingTop", "paddingBottom", "paddingLeft", "paddingRight", "borderLeftWidth", "borderRightWidth", "borderTopWidth", "borderBottomWidth"], n = {boxSizing: "border-box" == t.boxSizing};
    ...
    if (e) {
        var n = Math.max(e.offsetLeft - this.config.task_scroll_offset, 0), a = e.offsetTop - (this.$task_data.offsetHeight - this.config.row_height) / 2;
        this.scrollTo(n, a)
    }
    ...
    if (this._is_render_active()) {
        ...
        var t = this.copy(this._restore_scroll_state()), e = null;
    ...
    function t(t) {
        ...
        var a = e ? -20 * t.deltaX : 2 * t.wheelDeltaX, i = e ? -40 * t.deltaY : t.wheelDelta;
        if (a && Math.abs(a) > Math.abs(i)) {
            if (n.x)return !0;
            if (!gantt.$scroll_hor || !gantt.$scroll_hor.offsetWidth)return !0;
            var s = a / -40, r = gantt.$task.scrollLeft, o = r + 30 * s;
            ...
            var _ = gantt.$scroll_ver.scrollTop, d = gantt.$scroll_ver.scrollTop + 30 * s;
    ...
    this.event(this.$scroll_hor, "scroll", function () {
        ...
        var t = gantt.$task.scrollLeft, e = gantt.$scroll_hor.scrollLeft;
        e != t && (gantt.$scroll_hor.scrollLeft = t)
    }), this.event(this.$task_data, "scroll", function () {
        var t = gantt.$task_data.scrollTop, e = gantt.$scroll_ver.scrollTop;
    ...
    var t = this._get_grid_width(), e = Math.max(this._x - t, 0), n = Math.max(this._y - this.config.scale_height, 0), a = this.config.scroll_size + 1, i = Math.max(this.$task_data.offsetWidth - a, 0), s = this.config.row_height * this._order.length, r = this._get_resize_options(), o = this._scroll_hor = r.x ? !1 : i > e, _ = this._scroll_ver = r.y ? !1 : s > n, d = {
        x: !1,
        y: !1,
        scroll_size: a,
        x_inner: i + t + a + 2,
    ...
    var e = gantt._get_target_node(t), n = gantt._getClassName(e);
    ...
    for (var a = gantt._get_target_node(t), i = ""; a;) {
        if (i = gantt._getClassName(a)) {
            ...
            if (s >= 0) {
                if (!n)return a;
                var r = 0 === s || !gantt._trim(i.charAt(s - 1)), o = s + e.length >= i.length || !gantt._trim(i.charAt(s + e.length));
                if (r && o)return a
            }
        }
        a = a.parentNode
    ...
    for (var e = this.$grid_data.childNodes, n = this.config.task_attribute, a = 0; a < e.length; a++)if (e[a].getAttribute) {
    ...
    cache: !0, method: "get", parse: function (t) {
        ...
        2 == arguments.length && (n = e, e = ""), this._call("POST", t, e, !0, n, {url: t, postData: e})
    }, _call: function (t, e, n, a, i, s, r) {
        var o = window.XMLHttpRequest && !gantt.env.isIE ? new XMLHttpRequest : new ActiveXObject("Microsoft.XMLHTTP"), _ = null !== navigator.userAgent.match(/AppleWebKit/) && null !== navigator.userAgent.match(/Qt/) && null !== navigator.userAgent.match(/Safari/);
        if (a && (o.onreadystatechange = function () {
            ...
            }), "GET" != t || this.cache || (e += (e.indexOf("?") >= 0 ? "&" : "?") + "dhxr" + (new Date).getTime() + "=1"), o.open(t, e, a), r)for (var d in r)o.setRequestHeader(d, r[d]); else"POST" == t.toUpperCase() || "PUT" == t || "DELETE" == t ? o.setRequestHeader("Content-Type", "application/x-www-form-urlencoded") : "GET" == t && (n = null);
    ...
    function r(t, a, i) {
        ...
        var o = Math.abs(Math.floor(((window.innerWidth || document.documentElement.offsetWidth) - r.offsetWidth) / 2)), _ = Math.abs(Math.floor(((window.innerHeight || document.documentElement.offsetHeight) - r.offsetHeight) / 2));
    ...
    function l(t, e, n) {
        return "object" != typeof t && ("function" == typeof e && (n = e,
            e = ""), t = {text: t, type: e, callback: n}), t
    }
    ...
    init: function () {
        ...
        var a = n.getFullYear(), i = Math.round((n.getTime() - new Date(a, 0, 1).getTime()) / 864e5), s = 1 + Math.floor(i / 7);
        return s
...
function () {
    gantt.mixin(gantt.config, {
        ...
        lightbox: {
            sections: [{name: "description", height: 70, map_to: "text", type: "textarea", focus: !0}, {
                name: "time",
                type: "duration", map_to: "auto"
            }],
            project_sections: [{
                name: "description",
                height: 70,
        ...
        show_errors: !0
    }), gantt.keys = {edit_save: 13, edit_cancel: 27}, gantt._init_template = function (t, e, n) {
        ...
        var e = this.date.date_to_str, n = this.config;
        gantt._init_template("date_scale", !0), gantt._init_template("date_grid", !0, "grid_date_format"), gantt._init_template("task_date", !0), gantt.mixin(this.templates, {
            ...
            link_description: function (t) {
                var e = gantt.getTask(t.source), n = gantt.getTask(t.target);
                return "<b>" + e.text + "</b> &ndash;  <b>" + n.text + "</b>"
            },
            drag_link: function (t, e, n, a) {
                t = gantt.getTask(t);
                var i = gantt.locale.labels, s = "<b>" + t.text + "</b> " + (e ? i.link_start : i.link_end) + "<br/>";
    ...
    function i(t) {
        var e = gantt._task_area_pulls, n = gantt.getTask(t);
        if (n && gantt.isTaskVisible(t))for (var a in e)if (n = e[a][t], n && n.getAttribute("task_id") && n.getAttribute("task_id") == t) {
            var i = n.cloneNode(!0);
            return g = n, e[a][t] = i, n.style.display = "none", i.className += " gantt_drag_move ", n.parentNode.appendChild(i), i
        }
    }

    var s, r = 0, o = !1, _ = !1, d = null, l = null, g = null;
    this._gantt_touch_event_ready || (this._gantt_touch_event_ready = 1,
        gantt.event(gantt.$container, t[0], function (t) {
            if (!n(t) && o) {
                ...
                if (i && d) {
                    var g = d.pageX - i.pageX, h = d.pageY - i.pageY;
    ...
    function t(t, e) {
        var n = gantt.env.isIE ? "" : "%c", a = [n, '"', t, '"', n, " has been deprecated in dhtmlxGantt v4.0 and will stop working in v5.0. Use ", n, '"', e, '"', n, " instead. \nSee more details at http://docs.dhtmlx.com/gantt/migrating.html "].join(""), i = window.console.warn || window.console.log, s = [a];
